...
>>>>>>> 4f49380 (add Basic Payment Setup)
      ...
      .then(([statsData, volumeData, sessionsData, predictiveData]) => {
...
>>>>>>> f762cfe (Completed Phase 6 -heatmap history)
  ...
  const lineChartData = useMemo(() => {
    ...
    const height = 200;
    const padding = 30;
    const values = activeHistory.map((item) => item.estimated1Rm);
    const maxVal = Math.max(...values, 10);
    const minVal = Math.min(...values, 0);
    const range = maxVal - minVal === 0 ? 10 : maxVal - minVal;

    const points = activeHistory.map((item, index) => {
      const x = padding + (index / Math.max(activeHistory.length - 1, 1)) * (width - 2 * padding);
      const y = height - padding - ((item.estimated1Rm - minVal) / range) * (height - 2 * padding);
      return { x, y, value: item.estimated1Rm, date: item.date };
    });

    const pathD = points.length ? `M ${points[0].x} ${points[0].y} ${points.slice(1).map((point) => `L ${point.x} ${point.y}`).join(" ")}` : "";
  ...
  const maxVolume = useMemo(() => {
    ...
    return values.length ? Math.max(...values, 1000) : 1000;
  }, [volumeByMuscle]);

  const selectedForecast = useMemo(() => {
    if (!predictive?.strengthForecasts?.length) return null;
...
>>>>>>> 4f49380 (add Basic Payment Setup)
          ...
          <div className="feature-spotlight-grid">
            ...
            <div className="feature-story-card">
              ...
              <p className="feature-story-copy">{predictive.consistency.insight}</p>
            </div>
            <div className="feature-story-card">
              <div className="feature-kicker">Body composition</div>
              <p className="feature-story-copy">{predictive.bodyRecommendation}</p>
            </div>
          </div>
<<<<<<< HEAD
          <div className="feature-story-card">
          ...
          <div className="content-grid">
            <section className="panel" style={{ display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "center", marginBottom: "1rem", flexWrap: "wrap" }}>
                <div>
                  <div className="metric-label">Selected lift</div>
                  <div style={{ fontWeight: 700, color: "var(--text)" }}>{selectedExercise || "Choose an exercise"}</div>
                </div>
                <div>
                  <select
              ...
              {lineChartData ? (
                <div style={{ position: "relative", width: "100%", overflowX: "auto" }}>
                  <svg viewBox={`0 0 ${lineChartData.width} ${lineChartData.height}`} style={{ width: "100%", height: "auto", minWidth: "450px" }}>
                    <line x1="30" y1="30" x2="470" y2="30" stroke="var(--border)" strokeWidth="1" strokeDasharray="4,4" />
                    <line x1="30" y1="100" x2="470" y2="100" stroke="var(--border)" strokeWidth="1" strokeDasharray="4,4" />
                    <line x1="30" y1="170" x2="470" y2="170" stroke="var(--border)" strokeWidth="1" />
                    <path d={lineChartData.pathD} fill="none" stroke="var(--accent)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                    {lineChartData.points.length > 0 ? (
                      <path d={`${lineChartData.pathD} L ${lineChartData.points[lineChartData.points.length - 1].x} 170 L ${lineChartData.points[0].x} 170 Z`} fill="url(#chart-gradient)" opacity="0.12" />
                    ) : null}
                    <defs>
                    ...
                    {lineChartData.points.map((point, index) => (
                      <g key={index}>
                        <circle cx={point.x} cy={point.y} r="5" fill="#16110a" stroke="var(--accent)" strokeWidth="3" />
                        <text x={point.x} y={point.y - 12} textAnchor="middle" fill="var(--text)" fontSize="10" fontWeight="700">
                          {point.value}kg
                        </text>
                        <text x={point.x} y="188" textAnchor="middle" fill="var(--text-soft)" fontSize="9">
                ...
                <div className="panel-empty">Log an exercise to visualize its estimated 1RM progression.</div>
              )}

              <div style={{ marginTop: "1.5rem" }}>
                <div className="helper-text" style={{ lineHeight: 1.6 }}>
                  💡 <strong>1RM (One-Rep Max)</strong> calculation uses the Epley formula: <code>weight × (1 + reps/30)</code>.
                </div>
                {selectedForecast ? (
                  ...
                  return (
                    <div key={muscle} style={{ display: "grid", gap: "0.35rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.9rem" }}>
                        <span style={{ textTransform: "capitalize", fontWeight: 700 }}>{formatMuscleLabel(muscle)}</span>
                      ...
                      <div className="progress-track" style={{ height: "0.85rem", marginTop: 0 }}>
                        <div className="progress-fill" style={{ width: `${percent}%`, transition: "width 500ms ease-out" }} />
                      </div>
                    </div>
                  );
                })}
